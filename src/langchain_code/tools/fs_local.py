...
def _allow(apply: bool, action_desc: str) -> tuple[bool, str]:
    ...
    return False, f"dry-run (apply=False): {action_desc}"
...
def make_read_file_tool(project_dir: str):
    ...
    def read_file(path: str) -> str:
        """
        Read the full contents of a file. Returns error text if missing.
        """
        p = _rooted(project_dir, path)
...
def make_write_file_tool(project_dir: str, apply: bool):
    ...
    def write_file(path: str, content: str) -> str:
        """
        Overwrite a file with new content (creates parents automatically).
        Shows a unified diff vs the old file. If apply=False → dry-run only.
        """
        p = _rooted(project_dir, path)
        old = ""
        if p.exists():
        ...
        allowed, reason = _allow(apply, f"write {path}")
        if not allowed:
            return f"{reason}\nDiff:\n{diff}"
...
def make_edit_by_diff_tool(project_dir: str, apply: bool):
    ...
    def edit_by_diff(path: str, original_snippet: str, replaced_snippet: str) -> str:
        ...
        Shows the diff. If apply=False → dry-run only.
        """
        p = _rooted(project_dir, path)
        if not p.exists() or not p.is_file():
            return f"{path} not found or not a file."
        ...
        allowed, reason = _allow(apply, f"edit {path}")
        if not allowed:
            return f"{reason}\nDiff:\n{diff}"
...
def make_delete_file_tool(project_dir: str, apply: bool):
    ...
    def delete_file(path: str) -> str:
        """
        Delete a file. If apply=False → dry-run only.
        """
        p = _rooted(project_dir, path)
        if not p.exists():
            return f"{path} not found."
        ...
        allowed, reason = _allow(apply, f"delete {path}")
        if not allowed:
            return f"{reason}"
        p.unlink()
